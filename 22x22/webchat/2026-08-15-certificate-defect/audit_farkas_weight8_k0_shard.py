#!/usr/bin/env python3
"""Sharded exact no-symmetry Farkas audit for ALL current weight-8 k=0 children.

All four weight-8 lines are saturated. A child closes only if every exact
geometric identity subcase gets a Farkas certificate reconstructed over Q and
finally verified by Python integer arithmetic. Any failure leaves it open.
No symmetry is used.
"""
...
base=fk.base
NEW8=set(level8.NEW8)
ALL=[t for t in level8.CASES if t[2]==0]
assert len(ALL)==621
...
def main():
    ...
    payload={'n':22,'target':34,'k8':0,'symmetry':'none','shard':args.shard,
