...
def verify_upper_one(name, cert):
    ...
    assert upper < 32, (name, upper)
    print(f"upper bound verified for {name}: {upper} < 32, min cover {min_cover} at {min_point}")
...
def verify_upper_certificates():
    ...
    print("upper bound verified: D_mono(20) <= 31")


if __name__ == "__main__":
    verify_lower_config()
    verify_upper_certificates()
    print("verified frontier: 30 <= D_mono(20) <= 31")
